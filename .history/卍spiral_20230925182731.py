...
def loutus_x_to_inverse(n, thelta=0):
    return np.sqrt(2)**n*np.cos(n*np.pi/4+np.pi/2+thelta)


def loutus_y_to_inverse(n, thelta=0):
    return np.sqrt(2)**n*np.sin(n*np.pi/4+np.pi/2+thelta)
...
#             (loutus_x_to_inverse(n, alpha*np.pi/2), loutus_y_to_inverse(n, alpha*np.pi/2)), (loutus_x_from_inverse(n, alpha*np.pi/2), loutus_y_from_inverse(n, alpha*np.pi/2)), 'r')
# # anticlockwise
# for alpha in range(0, 4):
#     for n in range(-10, 9):
#         arc_inverse((loutus_x_center_inverse(n, alpha*np.pi/2), loutus_y_center_inverse(n, alpha*np.pi/2)),
#                     (loutus_x_from_inverse(n, alpha*np.pi/2), loutus_y_from_inverse(n, alpha*np.pi/2)), (loutus_x_to_inverse(n, alpha*np.pi/2), loutus_y_to_inverse(n, alpha*np.pi/2)), 'y')
