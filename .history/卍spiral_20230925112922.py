...
def loutus_x_center(n, thelta=0):
    return np.sqrt(2)**(n-1)*np.cos((n-1)*np.pi/4-3*np.pi/4+thelta)


def loutus_y_center(n, thelta=0):
    return np.sqrt(2)**(n-1)*np.sin((n-1)*np.pi/4-3*np.pi/4+thelta)
...
def loutus_x_to(n, thelta=0):
    return np.sqrt(2)**(n+1)*np.cos((n+1)*np.pi/4+np.pi/4+thelta)


def loutus_y_to(n, thelta=0):
    return np.sqrt(2)**(n+1)*np.sin((n+1)*np.pi/4+np.pi/4+thelta)
