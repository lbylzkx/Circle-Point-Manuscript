...
for n in range(-10, 10):
    arc((x_position(n-1, 3*np.pi/4), y_position(n-1, 3*np.pi/4)), (x_position(n+1, np.pi/4), y_position(n+1, np.pi/4))
        (x_position(n), y_position(n)), 'b')


# arc((x_position(8, thelta=0),y_position(8, thelta=0)))
