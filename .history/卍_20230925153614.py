...
for alpha in range(0, 4):
    for n in range(-10, 9):
        arc((loutus_x_center(n, alpha*np.pi/4), loutus_y_center(n, alpha*np.pi/4)),
            (x_position(n, alpha*np.pi/4), y_position(n, alpha*np.pi/4)), (loutus_x_to(n, alpha*np.pi/4), loutus_y_to(n, alpha*np.pi/4)))
